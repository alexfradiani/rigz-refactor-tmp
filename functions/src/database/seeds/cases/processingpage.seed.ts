...
export default class ProcessingPageSeed {
  ...
  async one(): Promise<void> {
    const carriersCount = 20;
    const loadsCount = 5;
    const ftCount = 5;

    const carrierSeed = new CarrierSeed(this.db);
    const carriers = await carrierSeed.many(carriersCount);
    console.log("carriers created");

    const userSeed = new UserSeed(this.db);
    const users = await userSeed.many(carriersCount);
    console.log("users created");

    const loadSeed = new LoadSeed(this.db);
    let loads: Load[] = [];
    for (let i = 0; i < carriersCount; i++) {
      const newLoads = await loadSeed.withCarrier(carriers[i], loadsCount);
    ...
    for (let i = 0; i < loads.length; i++) {
      await ftSeed.withLoad(loads[i], ftCount);
    }
